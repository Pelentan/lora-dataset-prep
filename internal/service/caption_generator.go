...
func (cg *CaptionGenerator) GenerateCaption(artifact *models.Artifact, image *models.TrainingImage) (string, error) {
	...
	// Get manufacturer from additional_properties if it exists
	var mfrName string
	if artifact.AdditionalProperties != nil {
		if mfrCodes, ok := artifact.AdditionalProperties["manufacturer_codes"].([]interface{}); ok && len(mfrCodes) > 0 {
			if mfrCode, ok := mfrCodes[0].(string); ok {
				mfrName, _ = cg.getLookupName("manufacturer_codes", &mfrCode)
			}
		}
	}
